...
interface GlobalCountrySelectorProps {
  ...
  onChange: (country: GlobalCountry) => void;
  placeholder?: string;
  className?: string;
...
export const GlobalCountrySelector: React.FC<GlobalCountrySelectorProps> = ({
  value,
  onChange,
  placeholder,
  className = "",
  ...
  const [filteredCountries, setFilteredCountries] = useState<GlobalCountry[]>(
    []
  );
  const [allCountries, setAllCountries] = useState<GlobalCountry[]>([]);
  ...
  useEffect(() => {
    ...
    const filtered = searchCountries(searchTerm, language);
    setFilteredCountries(filtered);
  }, [searchTerm, language, allCountries]);
  ...
  const handleSelect = (country: GlobalCountry) => {
    onChange(country);
    setIsOpen(false);
    setSearchTerm("");
  };
