...
exports.getShippingCompanies = async (req, res) => {
  try {
    ...
    const companies = await ShippingCompany.find(query)
      .select(
        "name code email phone address description offers logoUrl trackingUrlTemplate supportsLocal supportsInternational supportedCountries supportedLocalStates pricing internationalZoneRates internationalCountryZones volumetricDivisor codService expressService packagingService isActive",
      )
      .sort({ name: 1 });
